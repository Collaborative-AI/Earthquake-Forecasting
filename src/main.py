...
if __name__ == "__main__":
    ...
    # Canada Scraper
    input_path = os.path.join("src", "scraper", "Canada", "raw", "Canada-19850109-20240119.txt")
    ...
    # NOAA Scraper
    input_path = os.path.join("src", "scraper", "NOAA", "raw", "NCEI-WDS-Earthquakes.tsv")
    output_path = os.path.join("src", "scraper", "NOAA", "clean", "NOAA NCEI-WDS (0-2023).csv")
    separator = "\t"
    noaa = Scraper(input_path=input_path, output_path=output_path, separator=separator)
    noaa.find_quakes_txt(num_skips=0)
    
    # SoCal Scraper
    input_path = os.path.join("src", "scraper", "SoCal", "raw", "SearchResults.txt")
    output_path = os.path.join("src", "scraper", "SoCal", "clean", "SCEDC (1932-2023).csv")
